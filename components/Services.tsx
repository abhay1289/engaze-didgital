...
const Services: React.FC = () => {
  ...
  return (
    <section ref={targetRef} id="services" className="relative h-[600vh] bg-[#F8F9FB] transition-colors duration-500 selection:bg-teal-primary selection:text-white">
      <div className="sticky top-0 flex h-screen items-center overflow-hidden">

        <div className="absolute inset-0 opacity-[0.03] bg-[linear-gradient(to_right,rgba(15,23,42,0.06)_1px,transparent_1px),linear-gradient(to_bottom,rgba(15,23,42,0.06)_1px,transparent_1px)] bg-[size:50px_50px] pointer-events-none" />
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_120%,rgba(54,184,165,0.08),transparent_60%)] pointer-events-none" />
        ...
        <motion.div style={{ x }} className="flex h-full items-center pl-6 md:pl-12 lg:pl-24 w-max">
          ...
          <div className="w-[90vw] md:w-[500px] shrink-0 mr-12 md:mr-24 lg:mr-32 relative z-20 flex flex-col justify-center items-start px-2 md:px-0 text-left">
            <span className="text-teal-primary font-mono text-[10px] uppercase tracking-[0.3em] mb-6 block border border-teal-primary/30 py-1.5 px-4 rounded-full w-fit bg-teal-primary/10">The Engine</span>
            <h2 className="text-4xl md:text-5xl lg:text-6xl font-black leading-[0.85] mb-8 tracking-tighter text-[#0F172A] uppercase text-left">
              Our <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-br from-teal-primary to-teal-secondary relative">
                Stack
              </span>
            </h2>
            <p className="text-slate-500 text-base md:text-lg leading-relaxed font-light max-w-sm text-left border-l border-teal-primary/30 pl-6">
              A full suite of services designed to accelerate your digital growth.
            </p>
            <div className="mt-12 flex items-center gap-3 text-[10px] font-mono text-teal-primary uppercase tracking-[0.25em] bg-teal-primary/10 border border-teal-primary/20 px-4 py-2 rounded-full">
          ...
          <div className="flex gap-4 md:gap-8 lg:gap-16 pr-6 md:pr-12 lg:pr-48 relative z-20">
            {services.map((service, index) => (
              <div
                key={index}
                className="group relative w-[85vw] md:w-[500px] h-[60vh] md:h-[650px] bg-white backdrop-blur-3xl border border-slate-200 hover:border-teal-primary/30 transition-all duration-700 rounded-[2.5rem] md:rounded-[3rem] p-8 md:p-14 flex flex-col justify-between overflow-hidden shadow-sm shrink-0 text-left hover:bg-slate-50"
              >
                ...
                <div className="relative z-10 transition-transform duration-700 group-hover:-translate-y-2 text-left">
                  <div className="flex justify-between items-start mb-12 md:mb-20">
                    <span className="font-mono text-[10px] font-bold text-teal-primary bg-teal-primary/10 px-4 py-2 rounded-full border border-teal-primary/20 tracking-[0.2em]">
                      {service.id}
                    </span>
                    <div className="p-4 rounded-2xl bg-white text-teal-primary border border-slate-200 group-hover:scale-110 group-hover:bg-teal-primary/10 transition-all duration-500 shadow-sm group-hover:shadow-[0_0_20px_rgba(54,184,165,0.2)]">
                      {service.icon}
                    </div>
                  </div>
                  <h3 className="text-2xl md:text-3xl font-black text-[#0F172A] mb-4 md:mb-6 tracking-tighter leading-[0.9] uppercase text-left group-hover:text-teal-primary transition-colors duration-500">{service.title}</h3>
                ...
                <div className="relative z-10 translate-y-4 group-hover:translate-y-0 transition-all duration-700 text-left">
                  <p className="text-slate-400 text-sm md:text-base leading-relaxed mb-10 group-hover:text-slate-600 transition-colors font-light text-left">
                    {service.desc}
                  </p>
                  <button className="flex items-center gap-4 text-[11px] font-bold text-[#0F172A] group-hover:text-teal-primary transition-all duration-500 hover:gap-6 origin-left tracking-[0.2em] uppercase relative w-fit">
