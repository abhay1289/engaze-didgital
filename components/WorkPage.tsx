...
const WorkHero = () => {
    return (
        <section className="min-h-[50vh] flex flex-col justify-end px-6 md:px-12 pb-16 md:pb-24 bg-white pt-40 relative overflow-hidden">
            {/* Ambient Lighting */}
            <div className="absolute top-1/4 left-1/4 w-[40vw] h-[40vw] bg-teal-primary/5 rounded-full blur-[120px] pointer-events-none" />
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-12 text-left">
                    <div className="text-left">
                        <motion.h1
                            ...
                            className="text-5xl md:text-7xl lg:text-8xl leading-[0.85] font-black text-[#0F172A] tracking-tighter text-left uppercase"
                        >
                            ARCHIVE<span className="text-teal-primary">.</span>
                        </motion.h1>
                    </div>
                    <div className="flex gap-12 mb-4 text-left border-l border-slate-200 pl-8">
                        <div className="flex flex-col text-left">
                            <span className="text-slate-400 text-[10px] font-mono uppercase tracking-[0.2em] mb-2 text-left">Total Deployed</span>
                            <span className="text-3xl md:text-4xl text-[#0F172A] font-light text-left">54+</span>
                        </div>
                    </div>
                </div>
            </div>
...
const ProjectRow: React.FC<ProjectRowProps> = ({ project, index, setHoveredProject, onMobileTouch }) => {
    return (
        <motion.div
            ...
            className="group relative border-b border-slate-100 py-12 md:py-20 hover:bg-slate-50 transition-colors duration-500 cursor-pointer overflow-hidden text-left"
        >
            ...
            <div className="max-w-[1800px] mx-auto px-6 md:px-12 relative z-10 grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-8 items-start md:items-center text-left">
                <div className="md:col-span-2 flex flex-row md:flex-col gap-4 md:gap-2 items-center md:items-start justify-between md:justify-start text-left">
                    <div className="flex gap-4 items-baseline text-left">
                        <span className="font-mono text-sm text-slate-300 group-hover:text-teal-primary transition-colors text-left uppercase tracking-widest">0{index + 1}</span>
                        <span className="font-mono text-[10px] text-slate-300 uppercase tracking-[0.2em] text-left">{project.year}</span>
                    </div>
                ...
                <div className="md:col-span-5 text-left relative">
                    <h2 className="text-3xl md:text-5xl font-black text-[#0F172A] tracking-tighter leading-[0.9] text-left transition-transform duration-500 group-hover:translate-x-4 uppercase origin-left">
                        {project.client}
                    </h2>
                    <p className="text-base md:text-xl text-slate-500 mt-4 text-left font-light max-w-md transition-transform duration-500 group-hover:translate-x-4 delay-75">
                        {project.title}
                    </p>
                </div>
                <div className="col-span-1 md:col-span-3 flex flex-wrap gap-2 mt-4 md:mt-0 text-left">
                    {project.tags.map((tag, i) => (
                        <span key={i} className="px-4 py-2 bg-white border border-slate-200 rounded-full text-[10px] md:text-[10px] font-mono text-slate-500 text-left uppercase tracking-widest group-hover:border-slate-300 group-hover:text-[#0F172A] transition-colors duration-300">
                            {tag}
                        </span>
                    ))}
                </div>
                <div className="hidden md:flex md:col-span-2 justify-end">
                    <div className="w-16 h-16 rounded-full border border-slate-200 flex items-center justify-center group-hover:bg-teal-primary group-hover:border-teal-primary group-hover:text-white group-hover:scale-110 transition-all duration-500 bg-white text-slate-700">
...
const WorkPage: React.FC = () => {
    ...
    return (
        <div
            ...
            className="bg-white min-h-screen text-[#0F172A] transition-colors duration-500 cursor-default text-left relative selection:bg-teal-primary selection:text-white"
        >
            <GrainOverlay />
            <WorkHero />
            ...
            <div className="sticky top-20 md:top-24 z-40 bg-white/80 backdrop-blur-2xl border-y border-slate-100 py-4 text-left">
            ...
            {/* The List Wrapper */}
            <div className="min-h-[50vh] pb-32 pt-8 text-left border-t border-slate-100" onMouseLeave={() => setHoveredProject(null)}>
